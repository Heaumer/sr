...
\begin{itemize}
\item \textit{le-serveur}, une machine faisant office de serveur DNS, serveur DHCP et
passerelle ;
\item \textit{passerelle}, une machine Linux (Ubuntu) ayant le rôle de passerelle ;
\item \textit{client-bsd}, une machine BSD (FreeBSD) ;
\item \textit{backtrack}, une machine Linux (BackTrack).
\end{itemize}
...
statique, et l'on fait en sorte que la connexion soit effective
...
($x.x.x.1$) via les deux interfaces, et que l'on peut accéder à
Internet :
\begin{verbatim}
...
on ajoute une route par défaut pour faire passer le trafic par
...
les deux cas puisque les logiciels utilisés sont dans les deux cas identiques. 
Seule la gestion du routage/firewall change vraiment.
...
désactiver \textit{sendmail} pour éviter qu'ils ne se \og tapent \fg dessus.
\end{document}
